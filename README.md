...
## 2026-09-17 日期与 ILLIT 修复

页面现在按当前上海时间重新归类静态快照：早于今天的演出移入「已结束」，不再显示
购票按钮，也不计入在售/待开票数量及艺人角标。页面停留跨日、重新回到标签页时也会更新，
不必等待下一次采集或部署。今天的演出不会因为没有明确结束时间而提前归档。

确定性刷新新增 `config/artists.json` 中显式配置的 ILLIT 官方来源：韩国 NOL 正式演出票务页
和日本官网安可抽选公告，逐场采集仁川 10/17、10/18 与千叶东京湾 12/23、12/24、12/26、12/27。
开门/开演分别存储；开票时间保留韩国/日本 `+09:00`，页面开票提示转成北京时间。
`sale_status=scheduled` 配合 `sale_time` / 可选 `sale_end_time` 表示已核实的售票窗口：
开售前待开票，窗口内在售，抽选截止后显示「待后续开票」，不把抽选结束等同演出结束或售罄。
在售表示公告中的售票时间已到，实际余票仍以票务页为准。

这条链路每次刷新都会重新读取**已配置的公告页面**，不使用付费模型，不会自动发现所有新公告。
新一轮日本抽选/公售公告需要复核后更新来源配置；当前两个专用解析器要求完整解析韩国 2 场、
日本 4 场，页面结构变化或缺字段会阻止完整刷新发布。未配置官方来源的其他 K-Pop 艺人仍沿用
历史调研，页面顶部如实显示自动采集范围。未修改模型候选的隔离门禁。

回归测试：`python3 -m unittest discover -s tests -v`（前端行为测试需要 Node.js）。
测试覆盖旧快照跨日归档、页面计数同步、KST/JST 开票边界、抽选截止、6 场解析与重复刷新去重、
任一官方来源失败时禁止写入。
...
GitHub Actions（最长 90 分钟）
  ...
  · 确定性层：强制重新采集 ShowStart 与配置的官方来源；任一应采源失败即停止
  · 可选 candidate：默认关闭；显式选择 Kimi 也只写 runner temp artifact
  · 只合并确定性结果、构建 site/data.js 与 site/data.json
...
确定性层强制重新请求每个应采艺人的 ShowStart 数据及配置的官方公告。任一应采源失败都会在 merge、build 和
...
确定性刷新只把 ShowStart 与配置官方来源的新记录或明确更新合并进现有库存。本轮未命中的旧演出/舆情与
...
- 合并策略是保守的：本轮没搜索到或 enrich 失败时，旧记录不会仅因此被删除、降级或冒充本轮已复核；它保留既有 `verification_status`。过期演出在构建与浏览器展示时均按日期转入已结束；舆情超过 90 天会从页面隐藏。
